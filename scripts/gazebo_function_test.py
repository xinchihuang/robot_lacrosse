...
class Robot:
    def __init__(self, robot_name,initial_state,state="idle"):
        ...
        self.target_topic = rospy.Subscriber(f'/{self.robot_name}/image_raw', Image, self.tracking_callback,
                                             queue_size=1)
        ...
        try:
            if x_pixel == -1:
                ...
                if self.frame_count > self.skip_frame:
                    ...
                    self.odometry.append([0,0,0,present_time])
                if len(self.ball_memory) > self.min_frame_count:
                    print(len(self.ball_memory))
                    params = self.parabola_fitter.mono_fitter(self.ball_memory,odometry=self.odometry)
                    x_0, y_0, z_0, vx, vy, vz=params
                    print(x_0, y_0, z_0, vx, vy,vz)
                    impact_t=-x_0/vx
                    impact_point=self.parabola_fitter.parabola_model_time(params,impact_t)
                    print(impact_point)
...
class Ball:
    ...
    def reset_callback(self,data):
        ...
        if command.state == "reset":
            ...
            state_msg.pose.position.z = 2.25
            state_msg.twist.linear.x = 8.0
            state_msg.twist.linear.y = 1.0
