...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        ### transferring pose data
        self.broadcast_socket=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 1)

        # Set socket to broadcast mode
        self.broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
        self.broadcast_address = '255.255.255.255'  # Change this to your broadcast address
        self.broadcast_port = 12345  # Change this to your desired port

        # self.broadcast_queue = queue.Queue(maxsize=65535)
...
#Unpack Mocap Data Functions
    ...
    # def __broadcast_thread_function(self, in_socket):
    #     while True:
    #         time.sleep(0.05)
    #         message_to_broadcast=self.broadcast_queue.get()
    #         self.broadcast_queue.queue.clear()
    #         in_socket.sendto(message_to_broadcast.encode(), (self.broadcast_address, self.broadcast_port))
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            #### modified for lacrosse task only
            if self.lacrosse_listener is not None:
                self.lacrosse_listener(mocap_data)

            #### detect robot pose
            # message_to_broadcast=centralized_control(mocap_data)

            # self.broadcast_queue.put(message_to_broadcast)
    ...
    def run( self ):
        ...
        # self.broadcast_thread = Thread(target=self.__broadcast_thread_function, args = (self.broadcast_socket,))
