...
class Simulation:
    def __init__(self, max_velocity=0.5,stop_thresh=0.00,max_simulation_time_step = 1000):
        ...
        self.command_topic= rospy.Subscriber("command", String, self.command_callback)
    ...
    def command_callback(self, data):
        if data.data=="reset" or data.data=="1":
            ...
            state_msg.pose.position.y = 0
            state_msg.pose.position.z = 5.65
            state_msg.twist.linear.x = 1.0
            ...
            self.set_state(state_msg)
        elif data.data=="throw" or data.data=="2":
            set_joint_position(self.pub_revolute2, 1.0)
            set_joint_position(self.pub_revolute6, 1.0)
            set_joint_position(self.pub_revolute10, 0.0)
        rospy.loginfo("接收到来自 topic1 的数据: %s", data.data)
